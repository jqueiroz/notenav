...
lib/notenav.sh      # Full implementation (~7000 LOC)
config/
...
**Shell portability (Linux + macOS + FreeBSD):** Never use `sed -i` – write to a temp file and `mv`. See [GUIDELINES.md](GUIDELINES.md#portability-linux--macos--freebsd) for other portability traps.
