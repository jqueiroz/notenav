...
Before the manual audit, run ShellCheck on all bash source files:

```bash
nix-shell -p shellcheck --run "shellcheck -s bash bin/nn lib/notenav.sh curl/install.sh"
```

Report all warnings (severity "warning" and above). Info-level findings (SC2016 for intentional AWK strings, SC2086 for intentional word-splitting in `_in_array` calls) can be noted briefly but are expected.
...
- **Dependency checking**: version requirements verified at startup (fzf 0.45+, bash 4+, yq-go not yq-python)
