...
- **I4 – Accepted normalizations:** a rewritten file whose last line had no terminator gains one (pre-existing awk `print` behavior); unclosed frontmatter (no closing fence within the 200-line cap) makes the write a refused, byte-identical no-op — the rewriters pre-scan for the closing fence before touching anything; CR-only (classic Mac) files are treated as having no frontmatter.
