...
- **I2 – Preserving writes:** in-place note rewrites must be byte-preserving on untouched lines (each line keeps its own terminator; a BOM stays at byte 0, including when a frontmatter block is prepended). Lines notenav inserts or replaces use the file's EOL style, detected from line 1 (schematically `if (/\r$/) eol="\r"`, folded into the rewriters' line-1 fence rule). File permissions are preserved by stamping the note's mode on the temp file before the rename (`stat -L -c '%a' … || stat -L -f '%Mp%Lp' …`). Never normalize a whole file. (Rewrites replace the inode – the repo-wide tmp+`mv` pattern – so hard links to notes are not preserved; that is long-standing house behavior, not part of this invariant.)
...
- **I4 – Accepted normalizations:** a rewritten file whose last line had no terminator gains one (pre-existing awk `print` behavior); unclosed frontmatter (no closing fence within the 100000-line fm_cap) makes the write a refused, byte-identical no-op — the rewriters pre-scan for the closing fence before touching anything; CR-only (classic Mac) files are treated as having no frontmatter.
...
- **Use the modal pattern.** notenav has eight input modes: command mode (plain keys), change mode (`c` prefix), filter mode (`f` prefix), mark mode (`m` prefix), display mode (`z` prefix), help mode (`?`), search mode (`/`), and content search mode (`/?`). Fit new bindings into this structure.
...
- **Double-press cancels.** Pressing a modal prefix key twice (`cc`, `ff`, `zz`) must exit that mode – identical to pressing the prefix then escape. This keeps mode entry/exit symmetric and discoverable. Exception: `mm` toggles the mark on the current item instead of cancelling, since mark mode uses `m` as both prefix and action.
