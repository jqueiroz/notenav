...
- `nn doctor` reports the automatic poll fallback on WSL2 `9p`/`v9fs` mounts and warns when host or remote changes may not generate watch events on `virtiofs` or SMB/CIFS mounts.
- On WSL2 `9p`/`v9fs` Windows-drive mounts, where Windows-side edits do not reliably produce inotify events, `refresh.mode = "watch"` now transparently falls back to `poll` at startup, with a one-time "using poll refresh" note in the border. DrvFS on WSL1 remains on watch because it supports notifications; other uncertain filesystems (`virtiofs`, SMB/CIFS) are left on watch and flagged by `nn doctor`.
