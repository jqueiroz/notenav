...
- `nn doctor` warns when a notebook under `refresh.mode = "watch"` sits on a filesystem where inotify/fswatch deliver no change events (WSL's `/mnt/c` — `9p` on WSL2, `drvfs` on WSL1, `virtiofs` on newer setups — or SMB/CIFS mounts), recommending poll mode; previously the watcher ran but silently never fired there.
- On `9p`/`drvfs` filesystems (WSL Windows-drive mounts), which have no inotify support at all, `refresh.mode = "watch"` now transparently falls back to `poll` at startup — a dead watcher becomes working auto-refresh — with a one-time "using poll refresh" note in the border. Other inotify-uncertain filesystems (`virtiofs`, SMB/CIFS) are left on watch and only flagged by `nn doctor`, since a runtime auto-switch could wrongly demote a setup where local inotify does work.
- Editing a note (the `e` key) now updates the list immediately under `refresh.mode = "poll"` too; previously only manual mode reindexed synchronously, so in poll mode your own edit could keep showing the old value for up to `poll_interval` seconds.
...
- Empty strings no longer surface raw bash "bad array subscript" errors: a lone empty argument (`nn ""`) gets the clean not-a-preset message, `defaults.sort_by = ""` (no sort) works in ad-hoc queries, and an empty string in a workflow's type/status/priority `values` list is refused at startup with a diagnostic naming the section (`nn doctor` warns about it instead of crashing).
- Bulk edit now hints that a failed note ("N failed") was likely locked by another program or on an unwritable path — the same "locked or unwritable?" hint an inline action already gives — instead of a bare failure count (most relevant on `/mnt/c`, where a note open in a Windows app blocks the atomic write).
