...
- **`nn doctor` no longer skips its own `type_visibility = "typed_only"` warning** – the check read `$NN_TYPE_VISIBILITY`, but doctor calls `nn_load_config` without `nn_precompute_workflow`, so the variable is never populated in doctor's scope. The warning that flags "all notes will be hidden" silently never fired. Now reads from `nn_cfg` directly with the same default the runtime uses.
