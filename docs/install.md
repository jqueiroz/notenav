...
- **inotifywait** (Linux) or **fswatch** (macOS/FreeBSD): enables real-time auto-refresh when notes change on disk (`refresh.mode = "watch"`). Without either, auto-refresh is disabled (manual refresh is still possible via the `r` key). Install via `inotify-tools` (apt/dnf), `fswatch` (Homebrew), or `pkg install fswatch` (FreeBSD)
