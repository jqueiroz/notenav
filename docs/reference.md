...
4. **Workflow integrity:** validates the merged workflow definition:
   ...
   - **Refresh**: `mode`, `poll_interval`, and `auto_refresh_note_limit` have valid values; warns when `mode = "watch"` but no filesystem watcher is installed, reports the automatic poll fallback on WSL2 `9p`/`v9fs`, and warns when host or remote changes may not generate events on `virtiofs` or SMB/CIFS mounts; no unrecognized sub-keys
