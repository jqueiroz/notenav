...
- **`watch`** (default) – uses `inotifywait` (Linux) or `fswatch` (macOS/FreeBSD) to detect `.md` file changes in real time, with 1-second debouncing. If neither tool is installed, silently degrades to `manual` at runtime. On WSL2 `9p`/`v9fs` Windows-drive mounts, where Windows-side edits do not reliably produce inotify events, it transparently falls back to `poll` at startup (a one-time "using poll refresh" note appears in the border). DrvFS on WSL1 remains on watch because it supports notifications; `nn doctor` flags filesystems where host or remote changes may not fire (`virtiofs`, SMB/CIFS).
