...
- **Windows-side file notifications depend on the WSL filesystem.** On WSL2 `9p`/`v9fs` mounts, Windows-side edits do not reliably produce inotify events, so watch mode transparently falls back to **poll** (a one-time "using poll refresh" note shows in the border). DrvFS on WSL1 supports notifications and stays on watch. For filesystems where host or remote notifications are uncertain (`virtiofs`, SMB/CIFS), watch is left as-is and `nn doctor` suggests `"poll"` when appropriate. For the fastest experience, keep the notebook in the Linux filesystem (e.g. `~/notes`) and access it from Windows via `\\wsl$`.
...
**Repairing notes damaged by older versions:** notenav versions before 0.2.0 had a bug where editing a CRLF or BOM note prepended a duplicate frontmatter block (the note then lost its type/status in the list, and its real frontmatter showed as body text). `nn doctor` detects affected notes; `nn doctor --fix-frontmatter` repairs them by merging the two blocks (your latest edits win), writing a `.bak` backup next to each repaired note and refusing anything that doesn't match the known damage pattern. Review the result, then delete the `.bak` files. Detection is deliberately conservative — it will never touch a healthy note whose body merely resembles the damage, which also means a rare damaged note may go undetected (e.g. one whose original frontmatter held only unrecognized keys); such notes can be fixed by hand: delete the short leading `---` block, then re-apply its values to the real frontmatter below.
