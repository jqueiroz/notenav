...
- **File watching doesn't work on `/mnt/c`.** Linux inotify never fires for Windows-drive paths, so `refresh.mode = "watch"` silently does nothing there – use `"poll"` (or manual refresh). `nn doctor` detects this (it flags a notebook on a `9p`/`drvfs` filesystem under watch mode and tells you to switch to poll). For the fastest experience, keep the notebook in the Linux filesystem (e.g. `~/notes`) and access it from Windows via `\\wsl$`.
